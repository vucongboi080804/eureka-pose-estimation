...
ensemble, leave-scenes-out CV; AR 0.844, recall 0.940 at 10 mm). Matching,
...
117 required instances (visible ≥ 0.8), 110 matched at 10 mm. The 7 misses
(8 in the earlier draw `ablation_no_own_mask_run2.json`, which also missed
000014 #1 — see the flips below) split into two groups that need different
words:
...
of it at 10 mm. No required instance was left without a proposal (the fold
...
problem: **the whole 10 mm gap is
registration/verification (2; 3 in the earlier draw) and labels (5)**.
...
`train_gt_masks` row (0.974 at 10 mm) matches them only because it registers
...
185 predictions; 75 unmatched at 10 mm = 23 on ignore regions + 15 on
optional (< 0.8 visible) instances — both free — + 37 counted false
positives (precision 0.75). None of the 37 lies within 15 mm of a GT pose
(no double detections), 11 are half-turn flips of a real part (6 of them
within 10 mm of that part's centre: the losing rival kept because NMS keeps
different orientations), 26 are other wrong registrations of real proposals
straddling neighbours in piles (nearest GT 27–82 mm, centre 15–73 mm off);
none sits on background. Their median `score` is 0.41 against 0.86 for the
true positives, which is why thresholding `score` buys precision cheaply
(report: 0.84 at ≥ 0.4).
...
duplicate annotations no single-pose submission can match. Net: of the
0.060 recall gap at 10 mm, 0.043 is unreachable by design and 0.017 needs an
RGB cue in flip disambiguation (a further 0.009 comes and goes with the
RANSAC draw); the
remaining accuracy gap (2 mm recall 0.52) is the depth sensor, and holes are
the only lever the RGB gives.
...
| 000007 | 4 | 0.878 | 86 | 0 | yes | 2.7 | 0.42 | - |
...
| 000014 | 1 | 0.962 | 18 | 2 | yes | 3.5 | 0.87 | yolo11l_single, synthetic_only |
...
| 000019 | 2 | 0.933 | 5 | 1 | yes | 3.2 | 0.95 | - |
...
| 000026 | 3 | 0.954 | 16 | 2 | yes | 1.0 | 0.92 | - |
| 000026 | 4 | 0.938 | 8 | 2 | yes | 1.1 | 0.89 | - |
| 000030 | 0 | 0.930 | 17 | 2 | yes | 1.7 | 0.88 | - |
...
| 000033 | 5 | 0.838 | 87 | 0 | yes | 3.4 | 0.66 | - |
...
| 000041 | 0 | 0.819 | 8 | 2 | **no** | - | - | ensemble_run2, yolo11l_single, synthetic_only |
| 000041 | 1 | 0.949 | 7 | 2 | yes | 1.3 | 0.92 | - |
| 000041 | 3 | 0.924 | 20 | 2 | yes | 2.1 | 0.81 | - |
| 000041 | 4 | 0.944 | 46 | 2 | yes | 1.4 | 0.86 | - |
...
| 000047 | 3 | 0.929 | 7 | 1 | **no** | - | - | ensemble_run2, yolo11l_single, synthetic_only, gt_masks |
...
| 000054 | 0 | 0.902 | 6 | 2 | yes | 2.4 | 0.92 | - |
| 000054 | 1 | 0.936 | 35 | 2 | yes | 2.1 | 0.90 | - |
...
| 000058 | 4 | 0.835 | 27 | 0 | yes | 3.7 | 0.84 | yolo11l_single, gt_masks |
...
| 000059 | 2 | 0.920 | 16 | 2 | yes | 1.8 | 0.88 | - |
| 000059 | 3 | 0.931 | 17 | 2 | yes | 2.7 | 0.89 | - |
| 000059 | 4 | 0.863 | 19 | 1 | yes | 1.9 | 0.70 | - |
| 000059 | 5 | 0.901 | 26 | 1 | yes | 1.8 | 0.84 | - |
| 000059 | 6 | 0.923 | 44 | 2 | yes | 1.0 | 0.86 | - |
...
| 000041 | 0 | 0.819 | 73.0 (0.67) | 179 deg about Z (stem) | 6.6 | no | 0.85 (0.96, pass) | 0.83 (0.96, pass) | 000041_inst00_mssd73mm.png | flip: nearest pose is a half-turn about model Z (stem) |
...
| 000047 | 3 | 0.929 | 73.6 (0.67) | 177 deg about Z (stem) | 7.2 | no | 0.85 (0.93, pass) | 0.82 (0.83, pass) | 000047_inst03_mssd74mm.png | flip: nearest pose is a half-turn about model Z (stem) |

### Matched at 10 mm but not at 4 mm (5)
...
| bin | n | median mm | p75 mm | < 2 mm | < 4 mm |
|---|---|---|---|---|---|
| holes visible = 2 | 80 | 1.73 | 2.15 | 0.66 | 1.00 |
| holes visible = 1 | 8 | 2.95 | 3.42 | 0.38 | 0.88 |
| holes visible = 0 | 22 | 2.82 | 3.60 | 0.23 | 0.82 |
| tilt 0-15 deg | 56 | 1.87 | 2.59 | 0.59 | 0.95 |
| tilt 15-45 deg | 33 | 1.87 | 2.66 | 0.61 | 1.00 |
| tilt > 45 deg | 21 | 2.50 | 3.19 | 0.38 | 0.90 |

### Tally

- misses: 5 duplicate labels, 2 flips, 0 segmenter misses, 0 mislocalised, 0 other
- matched at 10 mm but not at 4 mm: 5 of 117
- predictions: 185; unmatched at 10 mm: 75 = 23 on ignore regions + 15 on optional instances (< 0.8 visible) + 37 false positives, of which 0 < 15 mm from a GT pose (second pose of a found part), 11 flips of a GT pose (> 150 deg), 26 other wrong poses of real parts
<!-- tables:end -->
