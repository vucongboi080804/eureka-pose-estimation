...
*The camera serves RGB-D frames with intrinsics; the pose service turns one frame into verified poses with a score; the cell layer picks a grasp and gates it; the controller picks, then rescans.*
...
| ------- | ---- | -------- | --------- |
| [`camera_service/`](camera_service/) | the sensor: frames, intrinsics, depth scale, frame ids | `/v1/frame`, `/v1/intrinsics`, `/preview.mjpg`, `/healthz`, `/metrics` | a dropped frame increments a counter; the stream continues |
| [`pose_service/`](pose_service/) | the model and the geometry: masks, registration, verification, `score` | `/v1/estimate`, `/healthz`, `/readyz`, `/metrics` | a bad frame returns an empty pose list with an error field |
| [`cell/`](cell/) | the decision: grasp choice, robot frame, pick policy, drift watch | library, driven by the cell loop (`cell/runner.py`) | no viable grasp returns reasons, and the policy escalates rescan → shake → stop |
...
[`cell/policy.py`](cell/policy.py) writes the cycle down as six states, so
...
`deploy.camera_service.record` (`CAM_SOURCE=session CAM_ROOT=<session dir>`).

On the board the pose service comes up as a systemd unit
([`jetson-nano/pose-service.service`](jetson-nano/pose-service.service));
the runbook is [`jetson-nano/README.md`](jetson-nano/README.md). For a
demonstration or a first check on new hardware,
[`jetson-nano/cell_demo.py`](jetson-nano/cell_demo.py) collapses the three
...
`deploy/pose_service/service.py` calls the same `detect_scene_hybrid` and
