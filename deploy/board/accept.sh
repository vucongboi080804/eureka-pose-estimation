...
#
#   deploy/board/accept.sh <user@host> [options]
#
...
# deploy/board/ACCEPTANCE.md. Read that before acting on a FAIL.
#
...
# bound is ~60 s (docs/offline-install.md). Unlike every other estimate in this
...
need "$prefix/deploy/board/bench.py" "the benchmark" \
...
try:
    from deploy.pose.config import ServiceConfig
...
remote_python "$SERVICE_PY" "$PREFIX" \
    "$PREFIX/deploy/board/config.nano.json" "$UNIT" </dev/null \
...
.venv/bin/python deploy/board/bench.py \
...
compare_against() {
    ...
    "$PY" "$ROOT/deploy/board/compare_bench.py" \
...
lines.append("machines, not board timings. Thresholds and what to do about "
             "each line:")
lines.append("deploy/board/ACCEPTANCE.md")
