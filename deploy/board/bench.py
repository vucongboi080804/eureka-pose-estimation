...
A timing without its hardware attached is a rumour: before the board was
reachable, the Jetson Nano numbers were an x86 desktop and a qemu-user
emulation, and the two disagreed by an order of magnitude. This script
produces one machine-readable record -- fingerprint, per-stage timings, peak
RSS, and the poses themselves -- from the identical command line on the
desktop, under emulation and on the board, so ``compare_bench.py`` can turn
"how close was the estimate" into a table. The board record that settled it
is ``results/bench/board_nano640.json``.

    cd <repo> && .venv/bin/python deploy/board/bench.py \
        --root . --split test --scenes 000001 000002 000015 --out bench.json

Emulated runs go through ``deploy/board/emulate.sh``, which applies the
