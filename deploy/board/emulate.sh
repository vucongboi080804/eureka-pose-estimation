#!/usr/bin/env bash
# Run deploy/board/bench.py inside the aarch64 image under the board's
...
#
#   deploy/board/emulate.sh --check     # print the command and the caveat
#   deploy/board/emulate.sh             # one scene, pick mode, ~4 min
#   SCENES="000001 000002" PICK=0 deploy/board/emulate.sh
#
...
DOCKER_ARGS=(
    ...
    -v "$RELEASE:/data:ro"
    -v "$ROOT/deploy/board:/app/deploy/board:ro"
    -v "$OUT_DIR:/out"
    --entrypoint python "$IMAGE"
    deploy/board/bench.py
...
if [ "${1:-}" = "--check" ]; then
    ...
    echo "image     $IMAGE   (built by deploy/board/Dockerfile; do not rebuild to change bench.py -- it is mounted)"
...
if ! docker image inspect "$IMAGE" >/dev/null 2>&1; then
    ...
    echo "  docker build --platform linux/arm64 -f deploy/board/Dockerfile -t $IMAGE $ROOT" >&2
    exit 1
fi
