# `board/` — bringing the cell up on a Jetson Nano 4 GB (JetPack 4.6)

The camera hands one registered RGB-D frame to **PoseService**
(`deploy/pose/`), a long-lived process that holds the CAD cloud and
...
| **Not yet verified** | the CUDA-10.2 torch path (every board number is CPU); the systemd unit on the board (the board ran the Docker image); [`../demo/cell_demo.py`](../demo/cell_demo.py) on the board (the board was offline); a real camera on the live path. |
...
The one-process demonstration the board is meant to run, [`../demo/cell_demo.py`](../demo/cell_demo.py), lives with the other demo code in `deploy/demo/`; it has not yet been run on the board.
...
docker run --rm --platform linux/arm64 -v "$PWD/wheelhouse-nano:/wh" \
    ...
    download -r deploy/board/requirements-jetson-nano.lock.txt pip -d /wh
```
...
.venv/bin/pip install --no-index --find-links wheelhouse-nano \
    -r deploy/board/requirements-jetson-nano.lock.txt
```
...
`deploy/board/config.nano.json` is the board profile: `seg_weights`
...
```bash
until .venv/bin/python -m deploy.pose.client health >/dev/null 2>&1
do sleep 2; done            # it prints the 503 body on each miss otherwise
.venv/bin/python -m deploy.pose.client health
```
...
```bash
.venv/bin/python deploy/demo/cell_demo.py --root /path/to/release --split test \
    --scenes 000001 --no-video
```

`cell_demo.py` runs the same code path in-process (arrays in, grasp pose
out) and prints the pick, the score and the policy decision per scene. To prove the *service* rather than the pipeline, send the same frame
over HTTP with the shipped client (`--help` for its exact flags; the
request/response contract itself is `deploy/pose/schema.py`):

```bash
.venv/bin/python -m deploy.pose.client estimate \
...
mkdir -p out_bench
.venv/bin/python deploy/board/bench.py --root /path/to/release --split test \
...
```bash
deploy/board/emulate.sh --check     # prints the command and the caveat
deploy/board/emulate.sh             # -> out_bench/emulated.json
```

Then diff the two — emulated first, board second:

```bash
.venv/bin/python deploy/board/compare_bench.py \
...
| Same, inside the CPU container | 1.8 GB peak | `docs/offline-install.md` |
...
`deploy/board/Dockerfile` (Ubuntu 18.04 aarch64 + deadsnakes 3.8 + the
...
| Poses correct offline, wrong through the service | Intrinsics or depth scale wrong on the live path | The service needs the *colour* camera's `K` and the `depth_scale` that turns ticks into metres, with depth registered to the colour image — same contract as `deploy/pose/adapter.py`. Re-run `deploy/demo/cell_demo.py --scenes <scene> --no-video` on a recorded scene to separate the camera from the pipeline. |
...
on the network, see `docs/offline-install.md`.
