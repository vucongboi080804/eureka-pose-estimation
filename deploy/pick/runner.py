...
five steps and nothing else:
    ...
    3. plan grasps on the returned poses (deploy/pick/grasp.py)
    4. ask the policy what to do (deploy/pick/policy.py)
    5. emit one JSON line and one human line

    python -m deploy.pick.runner --camera http://127.0.0.1:8081 \
        --pose http://127.0.0.1:8080 --cycles 5 --out cycles.jsonl
    python -m deploy.pick.runner --once
    python -m deploy.pick.runner --hand-eye hand_eye.json --cycles 0
...
deploy/pick/drift.py reserves for a cell with no success sensor. Set it
...
from ..camera.client import CameraClient, CameraClientError
from ..camera.frame import Frame, estimate_body
from ..pose.schema import FrameResult
    ...
    def __init__(self, camera_url: str, pose_url: str,
        ...
        """Args:
            ...
            hand_eye: ``T_base_camera`` as a
                :class:`~deploy.pick.frames.FramedTransform`, or None to
