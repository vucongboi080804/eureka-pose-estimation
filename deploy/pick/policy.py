...
from :mod:`deploy.pick.drift`; a cell that keeps picking on a
...
Run ``python -m deploy.pick.policy`` for a simulated sequence.
"""
...
#: (deploy/pose/config.py, accept_score) -- restated rather than
...
class CellFrame:
    ...
    def from_service(cls, frame_result: Any,
        ...
        """Adapt a ``deploy.pose.schema.FrameResult``."""
...
class PickHistory:
    ...
    #: Latest verdict from deploy.pick.drift, set by the cell.
    drift_verdict: str = drift.OK
