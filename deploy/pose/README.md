# `pose/` — the estimator as a service
...
.venv/bin/python -m deploy.pose.client estimate --scene test/000001    # exit 0 pick, 2 rescan, 1 failed
```

`--config` is a JSON of `ServiceConfig` fields ([config.py](config.py)); `POSE_<FIELD>` variables override it, `--host`/`--port` override the bind, `--once` answers one request. The defaults are the shipped two-segmenter configuration on `127.0.0.1:8080`; [config.nano.json](../board/config.nano.json) is one YOLO11n segmenter at 640, pick mode, gate 0.7.
...
| [adapter.py](adapter.py) | one registered RGB-D frame → `Scene` → poses; the seam a camera SDK plugs into, and the same constructor and detector calls as `scripts/run_pipeline.py` |
