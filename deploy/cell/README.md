...
```bash
.venv/bin/python -m deploy.cell.runner --once                                    # one cycle against the two local services
.venv/bin/python -m deploy.cell.runner --camera http://127.0.0.1:8081 --pose http://127.0.0.1:8080 --cycles 5 --out cycles.jsonl
.venv/bin/python -m deploy.cell.policy                                           # simulated state sequence; every module has such a self-check
```
