...
#
#   deploy/jetson-nano/preflight.sh <user@host> [--port N] [--identity KEY]
#                                   [--mode venv|docker]
#
# Examples:
#   deploy/jetson-nano/preflight.sh pose@192.168.102.39
#   deploy/jetson-nano/preflight.sh pose@nano.local --port 2222 --identity ~/.ssh/pose-nano
#
...
CONF="$REPO_ROOT/deploy/jetson-nano/$BOARD_CONFIG"
if [ -r "$CONF" ]; then
    missing=""
...
finish() {
    ...
    local next="deploy/jetson-nano/README.md step 2 -- build the wheelhouse, then copy the repo to $TARGET:$INSTALL_PREFIX"
    if [ -x "$REPO_ROOT/deploy/jetson-nano/provision.sh" ]; then
        next="deploy/jetson-nano/provision.sh $TARGET --port $PORT${IDENTITY:+ --identity $IDENTITY} --dry-run"
    fi
    if [ "$WARNS" -gt 0 ]; then
    ...
    case "$model" in
        *"Orin"*)
            ...
            fix "the install works here (aarch64, Python 3.8+), but every memory and latency" \
                "number in deploy/jetson-nano/README.md belongs to the Nano 4 GB profile" ;;
        *"Nano"*"2GB"*|*"Nano 2GB"*)
            warn identity "$desc"
        ...
        *)
            ...
            fix "the install works on Orin too, but every memory and latency number in" \
                "deploy/jetson-nano/README.md was measured for the Nano 4 GB profile" ;;
    esac
    remember "board    $desc"
fi
