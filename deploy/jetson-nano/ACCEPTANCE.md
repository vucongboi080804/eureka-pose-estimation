...
`deploy/jetson-nano/README.md` gets a Nano from a flashed SD card to a
...
```bash
deploy/jetson-nano/accept.sh pose@<board-ip>            # defaults below
deploy/jetson-nano/accept.sh pose@<board-ip> \
...
budget this cell is being built to. `deploy/OFFLINE.md` puts the cell's
...
Where it comes from: `deploy/pose_service/server.py`. The service binds its
...
- **The dataset is recorded, not live.** A pose that is right on
  ...
  `deploy/pick_demo.py` on a recorded scene separates the two.
