...
flowchart LR
    ...
    R[session]:::data --> S
    C[RealSense]:::data --> S
    S -->|GET /v1/frame| L[pick loop]:::check
...
```bash
CAM_SOURCE=session CAM_ROOT=sessions/test40 .venv/bin/python -m deploy.camera.server   # replay a recorded session
.venv/bin/python -m deploy.camera.server                                               # replay ./test, looping (the default)
.venv/bin/python -m deploy.camera.record --root . --split test --out sessions/test40 --fps 2   # record a split as a session
.venv/bin/python -m deploy.camera.client info                                          # intrinsics and source state
```
