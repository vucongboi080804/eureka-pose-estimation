...
what a shell script or a CI job calls to assert that it still is.
    ...
    python -m deploy.camera.client info
    python -m deploy.camera.client grab --out /tmp/capture
    python -m deploy.camera.client stream --n 20
...
the pose service can be pointed straight at:
    ...
    python -m deploy.pose.client estimate --scene /tmp/capture
...
class CameraClient:
    ...
    def frame_wire(self) -> Dict[str, Any]:
        ...
        :func:`deploy.camera.frame.estimate_body` -- because
