...
``depth_scale``) carry the names deploy/pose/schema.py already
...
#: from it, in the names deploy/pose/schema.py parses.
...
class Frame:
    ...
    def to_scene(self, scene_id: Optional[str] = None) -> Any:
        ...
        The conversion is the one ``deploy/pose/adapter.scene_from_arrays``
