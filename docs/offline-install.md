...
```bash
./setup.sh                      # .venv from requirements-lock.txt (--cpu | --cuda)
...
`requirements-lock.txt` is the exact closure of the runtime imports
...
```bash
docker build -f Dockerfile -t pose-est:cpu .     # needs internet, ~5 min
...
The board is its own target with its own pin set (Python 3.8 / aarch64,
open3d 0.18.0, CPU torch), Dockerfile, systemd unit and acceptance
scripts, all under [`deploy/board/`](../deploy/board/README.md). It was
measured on a real Nano 4 GB: 2.6–2.7 s per pick, 624 MB peak RSS
([`results/bench/board_nano640.json`](../results/bench/board_nano640.json));
the runbook there is the document to follow.
...
- `deploy/pose/adapter.py`: `scene_from_arrays(rgb_bgr, depth_u16, K,
...
- **Pick mode.** `pick=True` stops at the first pose scoring ≥ 0.8
  ...
  (`analysis/runtime.md`). `deploy/demo/cell_demo.py --scenes <scene> --no-video` runs exactly this
  path on a scene folder (arrays in, grasp pose out) — the check to run on
  a new machine or after a recalibration. Gate on `score`: ≥ 0.7 keeps precision
